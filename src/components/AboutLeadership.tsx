...
export default function AboutLeadership() {
  ...
  const leaders: Leader[] = [
    {
      ...
      quote: "Arun leads ASRAGEN's hardware and IoT work, from embedded systems and sensor design to automation prototypes and connected products.",
      about: "Arun leads the hardware engineering and IoT product roadmap at ASRAGEN, designing physical circuits, programming microcontrollers, and orchestrating connected sensor telemetry.",
      phone: '+91 9965241063',
    ...
    },
    {
      ...
      quote: "Vashanth works across software, product execution and digital systems, turning prototypes into usable applications.",
      about: "Vashanth heads the software team and system architecture at ASRAGEN, engineering digital platforms, databases, and admin dashboards that turn raw edge sensor streams into real-world insights.",
      phone: '+91 8220168887',
  ...
  return (
    <div className="leaders-section">
        ...
        {leaders.map((leader, index) => (
          <Reveal delay={index * 150} key={index}>
            <div className="leader-card" onClick={() => setActiveLeader(leader)}>
              ...
              <p className="tagline">"{leader.quote.substring(0, 90)}..."</p>
      ...
      {/* PROFILE DETAIL MODAL (REVEAL CARD WITH GLASSMORPHISM) */}
      <div 
        className={`profile-modal-overlay ${activeLeader !== null ? 'open' : ''}`}
      ...
      >
        {activeLeader && (
          <div className="profile-glass-card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="profile-title">{activeLeader.role}</div>
            ...
            <div className="profile-contact-container">
              <a href={`mailto:${activeLeader.email}`} className="profile-contact-item">
              ...
              <a href={`tel:${activeLeader.phone.replace(/\s+/g, '')}`} className="profile-contact-item">
                <svg className="profile-contact-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            ...
            <div className="profile-socials">
              {activeLeader.socials.github && (
                <a 
                  href={`https://github.com/${activeLeader.socials.github}`} 
                  target="_blank" 
              ...
              {activeLeader.socials.linkedin && (
                <a 
                  href={`https://linkedin.com/in/${activeLeader.socials.linkedin}`} 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  className="profile-social-btn"
                >
            ...
            <div className="profile-stats">
              <div className="profile-stat-col">
                <div className="profile-stat-num">{activeLeader.stats.num1}</div>
                <div className="profile-stat-label">{activeLeader.stats.label1}</div>
              </div>
              <div className="profile-stat-col">
                <div className="profile-stat-num">{activeLeader.stats.num2}</div>
                <div className="profile-stat-label">{activeLeader.stats.label2}</div>
              </div>
              <div className="profile-stat-col">
                <div className="profile-stat-num">{activeLeader.stats.num3}</div>
                <div className="profile-stat-label">{activeLeader.stats.label3}</div>
              </div>
            </div>
          </div>
        )}
